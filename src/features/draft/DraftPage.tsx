...
export function Component() {
  ...
  const pickResults = useDraftPicks(draftIds)
  const allPicks = pickResults.flatMap((r) => r.data ?? [])
  ...
  const picksByRound = useMemo(() => {
    ...
    allPicks
      .filter((p) => p.draft_id === latestDraft.draft_id)
      .forEach((pick) => {
  ...
  if (draftsLoading) return <LoadingScreen message="LOADING DRAFT..." />
  if (draftsError) return <ErrorState message="Failed to load draft data." onRetry={refetchDrafts} />
  if (!latestDraft)
    return (
      <PixelCard>
